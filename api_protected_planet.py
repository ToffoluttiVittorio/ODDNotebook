...
#
#
#url_test : geom = requests.get('https://api.protectedplanet.net/v3/protected_areas/search?token=0a694f50ddc7a276fccc40d80aa1556e&page=10')



############ POUR MONDE :

def convert_csv_to_monde(api_url, csv_filename='monde.csv', total_pages=10):
    # Création d'un DataFrame pandas
    df_list = []

    for page in range(1, total_pages + 1):
        # Requête à l'API avec la page actuelle
        response = requests.get(f'{api_url}&page={page}')
        json_data = response.json()

        # Boucle à travers les pays dans la réponse JSON
        for country_data in json_data['countries']:
            country_name = country_data['name']
            iso_3 = country_data['iso_3']
            statistics = country_data['statistics']
            region = country_data['region']

            # Création d'une ligne pour le DataFrame
            row = {
                'Name': country_name,
                'ISO_3': iso_3,
                'Continent':region['name'],
                'PA_Land_Area': statistics['pa_land_area'],
                'PA_Marine_Area': statistics['pa_marine_area'],
                'Land_Area': statistics['land_area'],
                'Percentage_PA_Land_Cover': statistics['percentage_pa_land_cover'],
                'Percentage_PA_Marine_Cover': statistics['percentage_pa_marine_cover'],
                'Marine_Area': statistics['marine_area']
            }
...
# Exemple d'utilisation
api_url = 'https://api.protectedplanet.net/v3/countries?token=0a694f50ddc7a276fccc40d80aa1556e'

convert_csv_to_monde(api_url)
